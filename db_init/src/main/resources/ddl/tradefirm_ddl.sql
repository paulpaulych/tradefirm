...
CREATE TABLE public.delivery_distribution(
 delivery_id   bigint NOT NULL REFERENCES public.delivery(delivery_id) on delete cascade,
 product_id    bigint NOT NULL REFERENCES public.product(product_id) on delete cascade ,
 sales_point_id bigint NOT NULL REFERENCES public.sales_point(sales_point_id) on delete cascade ,
 count        bigint NOT NULL
);
